...
import com.netcommlabs.sarofficenet.adapter.ArchivedAttendanceRequestAdapter;
 ...
 */
public class FragmentArchiveRequest extends Fragment implements View.OnClickListener, ResponseListener {
    private View viewMain;
    private FrameActivity mainActivity;
    ...
    public void onAttach(Context context) {
        ...
        if (context instanceof FrameActivity) {
            mainActivity = (FrameActivity) context;
        }
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        viewMain = inflater.inflate(R.layout.fragment_fragment_archive_request, container, false);
    ...
    private void validateNow(Dialog dialog) {
        if (tv_short_leave_list_from_date.getText().toString().equals("")) {
            Toast.makeText(mainActivity, "please choose from date", Toast.LENGTH_SHORT).show();
            return;
        }
        if (tv_short_leave_list_to_date.getText().toString().equals("")) {
            Toast.makeText(mainActivity, "please choose  to date", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void hitApiForPendingAttendanceDetail() {
        if (MyCalenderUtil.validDate(mainActivity, tv_pending_from_date.getText().toString(), tv_pending_to_date.getText().toString(), error_text)) {
            error_text.setVisibility(View.GONE);
            try {
                request = new ProjectWebRequest(getActivity(), getParamPendingAttendanceList(), UrlConstants.OD_ARCHIVE_REQUEST, this, UrlConstants.OD_ARCHIVE_REQUEST_TAG);
                request.execute();
            } catch (Exception e) {
                clearRef();
                e.printStackTrace();
    ...
    private JSONObject getParamPendingAttendanceList() {
      ...
      /*  Calendar c = Calendar.getInstance();
        ...
        if (FromDate.equalsIgnoreCase("")) {
            FromDate = "01/" + String.valueOf(mmonth) + "/" + String.valueOf(myear);
        }
        ...
        try {
            ...
            //   object.put("UserId", pref.getStringData(USER_ID));
    ...
    public void onSuccess(JSONObject obj, int Tag) {
        ...
        if (obj.optString("Status").equals("true")) {
            ...
            if (Tag == UrlConstants.OD_ARCHIVE_REQUEST_TAG) {
                ...
                try {
                    ...
                    lv_pending_request.setAdapter(new ArchivedAttendanceRequestAdapter(mainActivity, pendingList));
                } catch (Exception e) {
                }
            }
        } else {
