...
import com.netcommlabs.sarofficenet.adapter.ShortLeavePendingAdapter;
 ...
 */
public class FragmentShortLeavePending extends Fragment implements ResponseListener {
    ...
    private ArrayList<ShortLeavePendingModel> shortLeaveModelArrayList;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        // Inflate the layout for this fragment
         v = inflater.inflate(R.layout.fragment_short_leave_pending, container, false);
    ...
    private void hitApiForLeaveDetails() {
        try {
            request = new ProjectWebRequest(getContext(), getParamForLeaveDetails(), UrlConstants.SL_PENDING_LIST, this, SL_PENDING_LIST_TAG);
            request.execute();
        } catch (Exception e) {
            clearRef();
            e.printStackTrace();
    ...
    private JSONObject getParamForLeaveDetails() {
        ...
        if (FromDate.equalsIgnoreCase("")) {
            FromDate = "01/" + String.valueOf(mmonth - 2) + "/" + String.valueOf(myear);
        }
    ...
    public void onSuccess(JSONObject call, int Tag) {
        clearRef();
        if (Tag == SL_PENDING_LIST_TAG) {
            if (call.optString("Message").equalsIgnoreCase("Success")) {
                error.setVisibility(View.GONE);
                listView.setVisibility(View.VISIBLE);
                shortLeaveModelArrayList = new ArrayList<ShortLeavePendingModel>();
                ShortLeavePendingModel model;
                try {
                    JSONArray arr = call.getJSONArray("objSLGetDataRes");
                    for (int i = 0; i < arr.length(); i++) {
                        JSONObject object = arr.getJSONObject(i);
                        model = new ShortLeavePendingModel(object.optString("EmpName"),
                    ...
                    if (shortLeaveModelArrayList != null) {
                        ...
                        listView.setAdapter(new ShortLeavePendingAdapter(getContext(), shortLeaveModelArrayList));
                    }
