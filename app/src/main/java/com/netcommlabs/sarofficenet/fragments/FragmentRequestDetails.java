 ...
 */
public class FragmentRequestDetails extends Fragment implements View.OnClickListener, ResponseListener {
    ...
    private ArrayList<AttendanceRegularizationListModel> regulerizeList;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        viewMain = inflater.inflate(R.layout.fragment_request_details, container, false);
    ...
    private void hitApiForLeaveDetails() {
        try {
            request = new ProjectWebRequest(getContext(), getParamForLeaveDetails(), UrlConstants.GET_REGULERIZAE_DETAIL, this, UrlConstants.GET_REGULERIZAE_DETAIL_TAG);
            request.execute();
        } catch (Exception e) {
            clearRef();
            e.printStackTrace();
    ...
    public void onSuccess(JSONObject obj, int Tag) {
        ...
        if (obj.optString("Status").equals("true")) {
            ...
            if (Tag == UrlConstants.GET_REGULERIZAE_DETAIL_TAG) {
                regulerizeList = new ArrayList<AttendanceRegularizationListModel>();
                AttendanceRegularizationListModel model;
                try {
                    JSONArray arr = obj.getJSONArray("objARGetDataRes");
                    for (int i = 0; i < arr.length(); i++) {
                        JSONObject object = arr.getJSONObject(i);
                        model = new AttendanceRegularizationListModel(
                        ...
                        regulerizeList.add(model);
                    }
                    if (regulerizeList != null) {
                        //  header_row.setVisibility(View.VISIBLE);

                        lv_details.setAdapter(new AttendanceRegularizationListAdapter(getContext(), regulerizeList));
                    }
